...
;Variables
  ...
  !define PRODUCT_NAME "ElectrumMNX"
  !define PRODUCT_WEB_SITE "https://github.com/racquemis/electrum-mnx"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-mnx-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-mnx\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-mnx\*.*"
  File "c:\electrum-mnx\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mnx.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mnx.exe" "" "$INSTDIR\electrum-mnx.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\minexcoin-electrum" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\minexcoin-electrum" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\minexcoin-electrum" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\minexcoin-electrum\shell\open\command" "" "$\"$INSTDIR\electrum-mnx.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\minexcoin-electrum"
